	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < NUM_ITERATIONS; i++) {
			...
			if (DO_ADD) {
				...
				if (!res.equals(exp)) {
					System.out.println(
							String.format("FAIL: <%s> + <%s> expected <%s> but was <%s>", arg1, arg2, exp, res));
				} else {
					n_correct++;
				}
			}
...
//					System.out.println(
//					String.format("SUCCESS: <%s> - <%s> = <%s>", arg1, arg2, exp));
					n_correct++;
				}
			}
			if (DO_MUL) {
			...
			if (DO_DIV) {
				...
				if (exp == null) {
					...
					if (!res[0].equals(exp[0]) || !res[1].equals(exp[1])) {
						System.out.println(
								String.format("FAIL: <%s> / <%s> expected <%s remainder %s> but was <%s remainder %s>",
		...
		System.out.println(String.format("FINAL SCORE: %4.2f (%d out of %d correct)",
				100.0 * ((double) n_correct) / ((double) n_tests), n_correct, n_tests));
	}
