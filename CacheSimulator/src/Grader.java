...
public class Grader {
	...
	private void run() {
		...
		// Direct Mapped - 40% of grade, Fully associative - 30%, Set Associative - 30%
		// of grade
		int dmPassedAssertions = (dmAssertions - dmFailedAssertions);
		System.err.println(String.format("DIRECT MAPPED: PASSED %d/%d = %f%%", dmPassedAssertions, dmAssertions,
				(100.0 * dmPassedAssertions) / ((double) dmAssertions)));
		int faPassedAssertions = (faAssertions - faFailedAssertions);
		System.err.println(String.format("FULLY ASSOCIATIVE: PASSED %d/%d = %f%%", faPassedAssertions, faAssertions,
				(100.0 * faPassedAssertions) / ((double) faAssertions)));
		int saPassedAssertions = (saAssertions - saFailedAssertions);
		System.err.println(String.format("SET ASSOCIATIVE: PASSED %d/%d = %f%%", saPassedAssertions, saAssertions,
				(100.0 * saPassedAssertions) / ((double) saAssertions)));

		System.err.println(String.format("OVERALL SCORE (weighted) = %f%%",
				100.0 * (0.40 * dmPassedAssertions / ((double) dmAssertions)
						+ 0.30 * faPassedAssertions / ((double) faAssertions)
						+ 0.30 * saPassedAssertions / ((double) saAssertions))));
	}

	private void testFullyAssociative(int nBlocks, int nBytesPerBlock) {
		final String THIS_METHOD = new Object() {
		...
		// access 2nd block again, this should have been kicked out as lru, so accessing
		// it again
	...
	private void testSetAssociative(int nBlocks, int nBytesPerBlock, int associativity) {
		...
		for (int a = 0; a < associativity; a++) {
			...
			assertEquals(THIS_METHOD, "no more reads after accessing first set again", expectedReads,
					m_memory.getReadCalls());
		}

		// access the 2nd block, this should go in the other set and generate one more
		// memory access
		expectedReads++;
		for (int n = 0; n < nBytesPerBlock; n++) {
			int i = nBytesPerBlock + s_rand.nextInt(nBytesPerBlock);
		...
		assertEquals(THIS_METHOD, "additional memory read after accessing 2nd block", expectedReads,
				m_memory.getReadCalls());
		...
		assertEquals(THIS_METHOD, "no additional memory read after accessing 1st block", expectedReads,
				m_memory.getReadCalls());
		...
		assertEquals(THIS_METHOD, "no additional memory read after accessing 1st block for a 3rd time", expectedReads,
				m_memory.getReadCalls());
		...
		assertEquals(THIS_METHOD, "no additional memory read after accessing 2st block again", expectedReads,
				m_memory.getReadCalls());
	...
	private void testDirectMapped(int nBlocks, int nBytesPerBlock) {
		final String THIS_METHOD = String.format("%s(%d,%d)", new Object() {
		}.getClass().getEnclosingMethod().getName(), nBlocks, nBytesPerBlock);
		...
		// do this a bunch of times, should only require one read per block, read
		// sequentially through a chunk of memory
		for (int t = 0; t < 3; t++) {
		...
		// after all of that this should have just required one read per block, and each
		// address should have been read exactly once
		...
		// go a specific address in the next chunk of memory, guaranteed to not have
		// been loaded already
		...
		// each of the addresses in the original block should not have been read any
		// more
		for (int i = 0; i < nElements; i++) {
			assertEquals(THIS_METHOD, "per-address reads post 2nd chunk", 1, m_memory.getReadCount(base + i));
		}

		// now load the same element from the original chunk, should result in that
		// previous block being read again
		...
		for (int i = 0; i < nBytesPerBlock; i++) {
			...
			assertEquals(THIS_METHOD, "1st chunk per-address read counts post 2nd chunk", 2,
					m_memory.getReadCount(address));
		}
	...
	private void assertEquals(String method, String message, int expected, int actual, boolean ignore) {
		...
		if (expected != actual) {
			System.err.println(String.format("ERROR: assertion failure: %s: %s: expected <%d> but was <%d>", method,
